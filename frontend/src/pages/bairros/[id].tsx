...
import CustomizedInputsStyled from "@/components/text-field-custom";
...
export default function BairroEdit() {
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:3001/bairros/${router.query.id}`)
      .then((response) => response.json())
      .then((data) => setBairro(data));
  ...
  return (
    <>
      ...
      <Snackbar
            ...
            message="Bairro atualizado com sucesso, voltando à lista de bairros..."
          />
      {!bairro && <h1>Carregando informações</h1>}
    </>
  );
}
