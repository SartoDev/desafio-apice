...
export default function VendaEdit() {
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:3001/vendas/${router.query.id}`)
      .then((response) => response.json())
      .then((data) => {
        setVenda(data);
        setValorTotalVenda(parseFloat(data.valor));
      });
    fetch(`http://localhost:3001/pessoas`)
      .then((response) => response.json())
      .then((data) => setPessoaList(data));
    fetch(`http://localhost:3001/venda-itens?vendaId=${router.query.id}`)
      .then((response) => response.json())
      .then((data) => setVendaItemList(data));
  ...
  const handleChangeValue = (event: any) => {
    setVenda({ ...venda, valor: parseFloat(event.target.value) });
  };
  ...
  const handleChangeQuantidade = (event: any) => {
    setProdutoSelected({
      ...
      valorTotal: parseFloat(
        (event.target.value * produtoSelected?.valor).toFixed(2)
      ),
    });
  };
  ...
  const handleChangeValor = (event: any) => {
    setProdutoSelected({
      ...
      valorTotal: parseFloat(
        (event.target.value * produtoSelected?.quantidade).toFixed(2)
      ),
    });
  };
  ...
  const handleAddVendaItem = (event: any) => {
    if (
      vendaItemList.findIndex((itemList) => itemList.id == produtoSelected.id) >
      -1
    ) {
      return;
    }
  ...
  return (
    <>
          ...
          <div className="grid gap-4 w-6/12">
            <div className="flex gap-5">
              ...
              <TextFieldCustom
                ...
                variant="outlined"
                label="Data Venda"
                defaultValue={formatDate(venda?.dataVenda)}
                type="date"
              />
                ...
                <TableBody>
                  {vendaItemList.map((row: any) => (
                    <TableRow>
                      ...
                      <TableCell sx={{ color: "black" }}>{row.produto.nome}</TableCell>
                      <TableCell sx={{ color: "black" }}>{row.quantidade}</TableCell>
                      <TableCell sx={{ color: "black" }}>{row.produto.valor}</TableCell>
                      <TableCell sx={{ color: "black" }}>{(row.quantidade * row.produto.valor).toFixed(2)}</TableCell>
                      {
                        <TableCell>
                          <IconButton
      ...
      {!venda && <h1>Carregando informações</h1>}
    </>
  );
}
