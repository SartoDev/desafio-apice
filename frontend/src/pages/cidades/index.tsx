...
const columns: GridColDef[] = [
  ...
  { field: "siglaUF", headerName: "Sigla UF", width: 200 },
...
export default function Cidade() {
  ...
  const handleEditButton = (id: any) => router.push(`/cidades/${id}`);

  const handleDeleteButton = async (id: any) => {
    resetSnackbars()
    await new Promise(f => setTimeout(f, 500));
    const response = await fetch(`http://localhost:3001/cidades/${id}`, {
      method: "DELETE",
    });
  ...
  return (
    <div className="flex justify-center mt-7">
      ...
      <Snackbar
        ...
        autoHideDuration={3000}
        onClose={handleCloseSnackbar}
        message="Não foi possível excluir esta cidade"
      />
    </div>
  );
}
